...
## is drawn bigger.
##
## **1680 is a ceiling the full combat HUD imposes.** On the 1215x541 landscape
## reference it enlarges the old 2425-unit interface by roughly 44%, while the
## five actions, four abilities and scope rail still fit at once. Below this the
## individual targets are excellent but the complete control language no longer
## fits on screen. Verified at both phone orientations. [TUNE]
const PHONE_LOGICAL_WIDTH: float = 1680.0
...
static func factor_for(real: Vector2, base: Vector2, touch_layout: bool = false) -> float:
	...
	# failing against a phone-sized logical viewport nobody would ever see.
	if real.x < REAL_DISPLAY_WIDTH:
		return 1.0
	# Width alone misses landscape phones: the Android reference viewport is
	# 1215 wide and only 541 tall. Touch layouts still need readable scaling
	# after rotation.
	if not touch_layout and real.x >= SMALL_SCREEN_WIDTH:
		return 1.0
