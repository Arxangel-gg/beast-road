...
##
## Measured after: 0.31 0.39 0.42 0.34 0.36 0.36 0.44 0.45 0.47 0.53, a run
## mean of 0.403 against the 0.26-0.46 band, and a finale that is finally the
## hardest thing on the road - peaking at 0.74 across 64 bodies on the last
## wave before the Gatekeeper.
##
## **Acts IV to VI are a dip on purpose, and they used to be a slump.** The
## opening is tense because the purse is thin, not because the road is cruel;
## by Act IV the player has a defence that works, and a stretch that pays that
## off is the reward for having built it. But the first cut of this table left
## Act IV at 0.31 against Act I's 0.306 - a one percent margin over the floor
## `_judge_escalation` holds, which is not a designed breather, it is a slump
## that happened to clear the bar. Lifted to a clean eleven percent, so the
## shape reads tension, mastery, escalation rather than tension, sag, rescue.
##
